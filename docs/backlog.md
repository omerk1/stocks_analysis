...
- **`market_common` extraction (Done #32) is Step 0 of a larger, still-to-build effort**: gap/FVG detection, RSI/MACD/OBV divergence detection, multi-scale Fibonacci retracement/extension, and anchored VWAP — four modules in disjoint directories (`gaps/`, `divergences/`, `fibonacci/`, `avwap/`), each with its own config, SQLite-backed lifecycle tracking (writing to the new `data/derived/analysis.sqlite`), Plotly chart, and CLI. Full spec exists; none of the four are built yet.
