...
15. **Milestone 5: diagonal (RANSAC-style, log-price) trendlines** (same branch) — `DiagonalCandidate`/`generate_diagonal_candidates` (seed-pair fitting, ATR%-scaled inlier tolerance, least-squares refit, greedy dedup), `classify_events`/`score_line` generalized to work against a moving band (horizontal behavior unchanged, verified by the full suite throughout), `diagonal_penalty` implemented as a fit-quality RMS measure, diagonal lifecycle/dedup/plotting/CLI (`--diagonals`) wiring. Smoke-tested clean on real AAPL/T data (44 and 47 total lines respectively, diagonal lines competing meaningfully with horizontal ones) — structurally verified, first real-chart visual review round still pending. 151 tests passing. Full log: `docs/sr_lines_design_notes.md`.

## In progress / open questions

- Whether/how to source quarterly financials — still gated behind an undecided Polygon paid tier.
- **`sr_lines` milestone 5, diagonal real-chart review pending**: band width, dedup aggressiveness, the `diagonal_max_candidates` (30) cap (both AAPL and T hit it before dedup — may be too low, or the inlier tolerance too loose), and whether `max_diagonal_slope_atr_per_bar`'s log-slope interpretation is the right one, all need the same kind of visual-review pass the horizontal work went through. Milestone 6 (`as_of` dedicated test coverage) and milestone 7 (a systematic weight-tuning pass, now including diagonal-specific calibration) remain ahead. One open calibration question carried forward: `resilience`'s cap (1.0) can still be hit by a zone with enough events even after the time-decay fix — see `docs/sr_lines_design_notes.md`.
