...
**Related:** `docs/features/shared_outcome_statistics_design.md` §3 step 5 lists "gaps/divergences/fibonacci/avwap/sr_lines each getting their own lightweight aggregation step" as flagged-but-not-scoped. This doc is that scoping for `gaps` specifically — the first of the five. Modeled on the two existing backtest modules: `src/signals/patterns/backtest/evaluator.py` (migrated onto `market_common.stats.distribution_stats` per that same design doc's §3 step 2) and `src/signals/market_structure/backtest.py` (built on it from the start) — both now use the identical statistical treatment, so either is a fine template on that front.
...
- **Soft-close rate** — fraction reaching at least `SOFT_CLOSED` (≥80% filled) — a softer version of the same question, useful because a fill-rate/soft-close-rate split is exactly the kind of result that could inform whether `soft_close_pct`'s current default is set sensibly (it doesn't carry `config.py`'s own "starting point, not validated" caveat — that one's on `reaction_window_bars`, see §4 — but it's just as much a hand-picked default with no real-data check behind it yet).
