...
**"Extension (M4) with slope," generalized (2026-09-18, Track A, EXPLORATION_LOG.md) — is it a smooth function of *how far* extended, or true at only one decile?** M6.2's own first-slice run of this bullet (PREREGISTRATION.md) tested only the top/bottom distance decile with a binary slope sign. Generalized past that: full distance-decile range (0–9, both sides of the MA, not just the extreme), all four lookbacks, both SMA and EMA (`src/signals/moving_averages/distance_slope_surface.py`). **No smooth surface at any of the 8 (family, lookback) combinations** — the first-slice result sits at a single decile surrounded by noise in both directions, the "lone bright pixel" §6.7's plateau rule calls noise, not a wider effect. Longer lookbacks (150/200, both families) are additionally confounded by `slope_log_21`/distance collinearity rising to 0.77–0.93 — at that point the two axes are close to the same variable (§6.0's identity), leaving little independent 2D structure to find regardless.
