...
## Polygon free tier has no intraday data, and rate limit is the real scaling bottleneck
...
This is why yfinance is the intended intraday source, and why `bars_4h`
doesn't exist yet at all. The 5 req/min cap is also the actual bottleneck for
pulling a large ticker universe -- storage is trivial by comparison (roughly
80 KB/ticker for 2 years across all three timeframe tables), but the API
calls alone gate you to ~5 tickers/minute with no pacing/backoff logic beyond
the client's built-in retry-on-429.
