...
triangles + wedges, cup & handle + inverse so far), upserts them into the
derived DB, and prints a summary table. `--all` iterates every distinct
...
def main():
    parser = argparse.ArgumentParser(description="Detect chart patterns (double top/bottom, head & shoulders, triangles/wedges, cup & handle, ...)")
