...
- **Neutralized (the C2-equivalent robustness layer)**: `sector` and
  `vol_tercile` matched via `stats.inference.block_bootstrap_spread`'s
  existing stratified-match machinery (the same tool M1/M4 use for their
  own C2, reused rather than reimplemented). PREREGISTRATION.md describes
  this conceptually as demeaning/neutralization; in this implementation it
  is the same stratify-and-average mechanism M1/M4 already use, applied to
  a cross-sectional decile assignment computed once per date -- reused
  because it's the tested tool already in this repo, not a separate
...
NEUTRALIZATION_MATCH_COLS = ("sector", "vol_tercile")
...
def prepare(panel: pd.DataFrame, horizons: tuple[int, ...]) -> pd.DataFrame:
    ...
    neutralization match column (`vol_tercile`; `sector` is already on the
    panel). Returns a new frame; `panel` itself is not mutated.
    """
    working = panel.copy()
    for h in horizons:
...
def daily_rank_ic(panel: pd.DataFrame, feature_col: str, return_col: str, date_col: str = "date") -> pd.Series:
    ...
    def _corr(g: pd.DataFrame) -> float:
        sub = g[[feature_col, return_col]].dropna()
        if len(sub) < MIN_TICKERS:
            return float("nan")
        return sub[feature_col].corr(sub[return_col], method="spearman")

    return panel.groupby(date_col).apply(_corr, include_groups=False)


def decile_turnover_hurdle(
    panel: pd.DataFrame, decile_col: str, ticker_col: str = "ticker", round_trip_cost: float = ROUND_TRIP_COST
) -> dict:
    """Combined-legs annualized cost hurdle for a long-top/short-bottom
    decile portfolio, measured directly off `decile_col` via
    `stats.costs.signals_per_year` (entry+exit flip count -- the repo
    standard per PREREGISTRATION.md's 2026-09-09 M4 turnover-reconciliation
    addendum, not the entries-only convention M4 originally used).
    """
    is_top = panel[decile_col] == (N_DECILES - 1)
    is_bottom = panel[decile_col] == 0
    top = pd.array(is_top.where(panel[decile_col].notna(), pd.NA), dtype="boolean")
    bottom = pd.array(is_bottom.where(panel[decile_col].notna(), pd.NA), dtype="boolean")
    working = panel[[ticker_col]].copy()
    working["top_decile"] = top
    working["bottom_decile"] = bottom

    spy_top = signals_per_year(working, "top_decile", ticker_col=ticker_col)
    spy_bottom = signals_per_year(working, "bottom_decile", ticker_col=ticker_col)
...
def cell_result(
    ...
    working = panel.dropna(subset=[feature_col, return_col]).copy()
    ...
    cost = decile_turnover_hurdle(working, "decile")
