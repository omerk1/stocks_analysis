...
def test_trend_template_criteria_all_true_for_a_clean_bullish_stack():
    frame = m2._add_trend_template_criteria(_base_frame())

    for col in m2.CRITERIA_COLS:
...
def test_trend_template_criteria_are_na_not_false_during_sma200_warmup():
    ...
    result = m2._add_trend_template_criteria(frame)
...
def test_stack_fully_bullish_and_bearish_are_mutually_exclusive_and_correct():
    ...
    result = m2._add_stack_features(frame)
...
def test_evaluate_part_a_kill_criterion_fires_when_both_cells_below_floor():
    ...
    verdict = m2.evaluate_part_a_kill_criterion(incremental)
...
def test_ablation_subset_table_bitmask_partitions_rows_correctly():
    ...
    for i, col in enumerate(m2.CRITERIA_COLS):
    ...
    table = m2.ablation_subset_table(frame)
    ...
    row0_mask = int("".join("1" if frame.loc[0, c] else "0" for c in reversed(m2.CRITERIA_COLS)), 2)
...
def test_linear_attribution_recovers_planted_coefficients():
    ...
    for coef, col in zip(true_coefs, m2.CRITERIA_COLS):
    ...
    result = m2.linear_attribution(frame)
