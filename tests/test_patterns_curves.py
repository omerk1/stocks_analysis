...
def test_fit_quadratic_perfect_parabola_r2_is_one():
    ...
    prices = (x - 10) ** 2
    assert fit_quadratic(prices).r2 == pytest.approx(1.0)


def test_fit_quadratic_sharp_v_shape_scores_below_a_real_parabola():
    x = np.arange(21, dtype=float)
    prices = np.abs(x - 10) * 10
    assert fit_quadratic(prices).r2 < 0.95


def test_fit_quadratic_constant_prices_r2_is_one():
    # ss_tot == 0 -- trivially a perfect "fit", same convention as
    # trendlines.r_squared.
    assert fit_quadratic(np.full(10, 50.0)).r2 == 1.0
