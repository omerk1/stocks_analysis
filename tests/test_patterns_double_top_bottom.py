...
def test_reclaim_without_reaching_target_flags_failed_breakout():
    # Breaks the neckline, then reclaims back above it and never reaches
    # target before the resolution horizon elapses. The tail has to outrun
    # that horizon: a reclaim no longer resolves the match on its own, so a
    # short frame leaves it ACTIVE (right-censored) instead of failed.
    df = _chain(*_PREFIX, (118.0, 118.0, 1), (125.0, 128.0, 40))
    pivots = _double_top_pivots(df)
    config = _config()
